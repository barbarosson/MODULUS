...
export function SiteConfigProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (data) {
        console.log('✅ Site config fetched successfully:', {
          ...
          email_welcome_subject_en: data.email_welcome_subject_en,
          video_tutorial_links: data.video_tutorial_links,
        ...
        console.warn('⚠️ No site config data found');
      }
    } catch (error) {
    ...
    try {
      ...
      if (user) {
        ...
        if (profile && (profile.role === 'super_admin' || profile.role === 'admin')) {
          console.log('✅ Maintenance mode bypassed for role:', profile.role);
          setBypassMaintenance(true);
          return;
        }
      }
  ...
  const fetchMenus = async (retryCount = 0) => {
    try {
      ...
      if (error) {
        console.error('❌ Error fetching menus (attempt ' + (retryCount + 1) + '):', error);
      ...
      console.log('✅ Navigation menus fetched:', {
        total: data?.length || 0,
        visible: data?.filter(m => m.is_visible).length || 0,
        slugs: data?.map(m => m.slug).join(', ')
      });

      setMenus(data || []);
    } catch (error) {
      console.error('❌ Failed to fetch menus after retries:', error);
      setMenus([]);
    }
  };
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      console.log('🔵 SiteConfigProvider: Starting initial load...');
      setLoading(true);
      await Promise.all([fetchConfig(), fetchBanners(), fetchMenus()]);
      setLoading(false);
      setInitialized(true);
      console.log('✅ SiteConfigProvider: Initial load complete');
    };
    ...
    const checkInterval = setInterval(() => {
      if (menus.length === 0 && !loading) {
        console.warn('⚠️ Menus empty, forcing refresh...');
        fetchMenus();
      }
    }, 3000);
