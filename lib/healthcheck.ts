...
async function runThemeChecks(emit: (c: CheckResult) => void) {
  ...
  try {
    ...
    const { data: uiColors } = await supabase.from('ui_colors').select('*')

    const stylesCount = uiStyles?.length || 0
    const colorsCount = uiColors?.length || 0

    emit({
      id: 'theme-styles',
      category: 'theme',
      name: 'UI Stil Tanimlari',
      status: stylesCount > 0 || colorsCount > 0 ? 'pass' : 'info',
      severity: 'low',
      message: `${stylesCount} stil | ${colorsCount} renk tanimli`,
      details: [
        `UI stilleri: ${stylesCount}`,
        `UI renkleri: ${colorsCount}`,
      ],
      fixable: false,
    })
  } catch { /* skip */ }
