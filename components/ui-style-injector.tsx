...
export function UIStyleInjector() {
  useEffect(() => {
    const applyStyles = async () => {
      try {
        const [stylesRes, colorsRes] = await Promise.all([
          supabase.from('ui_styles').select('*'),
          supabase.from('ui_colors').select('*')
        ])
        ...
        if (colorsRes.data) {
          colorsRes.data.forEach((color: any) => {
            if (!color.property || !color.element_name || !color.value) return
        ...
        console.log('✅ UI Styles applied:', {
          styles: stylesRes.data?.length || 0,
    ...
    const applyButtonStyles = (styles: any[]) => {
      ...
      console.log('✅ Button styles applied:', buttonProps)
    }
    ...
    const handleTypographyUpdate = () => {
      console.log('🔄 Typography updated, reapplying styles...')
      applyStyles()
    }
    ...
    const stylesChannel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'ui_styles' }, (payload) => {
        console.log('🔄 UI Style updated:', payload.new)
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'ui_colors' }, (payload) => {
        console.log('🔄 UI Color updated:', payload.new)
        const color = payload.new as any
        if (!color.property || !color.element_name || !color.value) return
        const varName = `--${color.element_name}-${color.property.replace(/_/g, '-')}`
        document.documentElement.style.setProperty(varName, color.value)
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'ui_toggles' }, () => {
        console.log('🔄 UI Toggles updated, full refresh needed')
      })
      .subscribe()
